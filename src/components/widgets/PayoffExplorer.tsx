...
import { PayoffChart, positionPnl } from '../PayoffChart'
  ...
  return (
    <div className="widget">
      ...
      <div className="sliders">
        <Slider label="Страйк" value={strike} min={70_000} max={140_000} step={1000} display={usd(strike)} onChange={setStrike} />
        ...
        <Slider label="Дней до экспирации" value={dte} min={1} max={90} display={`${dte}д`} onChange={d => { setDte(d); setDaysPassed(dp => Math.min(dp, d)) }} />
        {props.showTime && (
          <Slider label="Дней прошло" value={Math.min(daysPassed, dte)} min={0} max={dte} display={`${Math.min(daysPassed, dte)}д`} accent="#f59e0b" onChange={setDaysPassed} />
        )}
      </div>
      <div className="stats">
