...
export function DeltaGamma() {
  ...
  const gammaScaled: [number, number][] = gammaPts.map(([x, y]) => [x, maxGamma > 0 ? y / maxGamma : 0])
  ...
  return (
    <div className="widget">
      ...
      <div className="sliders">
        ...
        <Slider label="Цена BTC" value={spot} min={60_000} max={150_000} step={500} display={usd(spot)} accent="#22c55e" onChange={setSpot} />
      ...
      <p className="hint">
        Уменьши количество дней до экспирации и посмотри, как кривая дельты превращается в «ступеньку»,
        а горб гаммы сжимается к страйку. Это и есть гамма-риск последних дней.
      </p>
    </div>
  )
}
